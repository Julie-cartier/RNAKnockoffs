...
load("/home/julie/Documents/Paper_codes/CUKPAP_EXPERIMENTS/Methods_comparison/R_files/table_perfx10.R") # tere are 100 iterations (not 10)
...
graph_power_fdp <- ggplot(df_power_fdp, aes(x=fdp, y=power, color = KO)) + 
   ...
   scale_shape_manual(values = c(7, 20, 15, 17, 18)) +  # Adjust the shape values according to your preference
   geom_ribbon(aes(ymin = power - std, ymax = power + std, group = KO, fill = KO), alpha = 0.15, colour = NA) +
...
load("/home/julie/Documents/Paper_codes/CUKPAP_EXPERIMENTS/Methods_comparison/R_files/MRC_table_perfx10.R")
...
graph_power_fdp <- ggplot(df_power_fdp, aes(x=fdp, y=power, color = KO)) + 
   ...
   # geom_pointrange( aes(ymin = power-std, ymax = power+std),linewidth = 1)+
   geom_point(aes(shape = KO), size = 4) +
   scale_shape_manual(values = c( 20, 15, 17, 18)) +  # Adjust the shape values according to your preference
   geom_ribbon(aes(ymin = power - std, ymax = power + std, group = KO, fill = KO), alpha = 0.15, colour = NA) +
   geom_line(data = subset(df_power_fdp, KO == 'LASSO'), aes(group = KO))+
   #geom_line(aes(group = KO), linewidth = 1.5)+
   labs(title = "Power vs FDP, k = 10, X from X real") +
