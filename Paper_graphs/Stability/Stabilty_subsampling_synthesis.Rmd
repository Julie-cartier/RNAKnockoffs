...
for (rep in 1:y.rep){
  ...
  prop_selection_0.5 = rowMeans(table.selected_genes.KOPI_0.5)
  print(length(which(colMeans(table.selected_genes.KOPI_0.2) == 0)))
  print(length(which(colMeans(table.selected_genes.KOPI_0.5) == 0)))
  
  prop_selection_0.2_0 <- prop_selection_0.2[ !prop_selection_0.2 == 0]# Remove features that have never been selected
  prop_selection_0.5_0 <- prop_selection_0.5[ !prop_selection_0.5 == 0]# Remove features that have never been selected
  
  data_0.2_KOPI <- data.frame(freq = prop_selection_0.2_0, status = as.factor(true_genes[ !prop_selection_0.2 == 0]))
  data_0.5_KOPI <- data.frame(freq = prop_selection_0.5_0, status = as.factor(true_genes[ !prop_selection_0.5 == 0]))
  ...
  ylim.value <- max(max_bin_count_KOPI_0.5, max_bin_count_KOPI_0.2, max_bin_count_LPLR)
  ...
  hist_KOPI_0.5 <- ggplot(data_0.5_KOPI, aes(x = freq, fill = status)) +
   ...
   theme(axis.text.x = element_text(size = 26, face = "bold"),
    ...
    scale_fill_manual(values =  c("#008899", "#FF5733", "#FFFF66"))
  #name = "KOPI (q = 0.5), n = 520") # Define colors for different status
  ...
  graph_min <- ggarrange(hist.genes, hist_KOPI_0.2, hist_KOPI_0.5, ncol = 3, common.legend = FALSE, font.label = list(size = 22)) 
  
  ggsave(file = str_glue("/home/julie/Documents/Paper_codes/Paper_graphs/Stability/Figures/Additional_figures/min_y_",rep,".png"), plot = graph_min, height = 9, width = 22)
  
  graph_oracle <- ggarrange(hist.genes.oracle, hist_KOPI_0.2, hist_KOPI_0.5, ncol = 3, common.legend = FALSE, font.label = list(size = 22)) 
  
  ggsave(file = str_glue("/home/julie/Documents/Paper_codes/Paper_graphs/Stability/Figures/Additional_figures/SS_y_",rep,".png"), plot = graph_oracle, height = 9, width = 22)
