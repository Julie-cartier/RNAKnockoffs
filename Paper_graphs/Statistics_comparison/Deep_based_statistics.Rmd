...
load("/home/julie/Documents/Paper_codes/CUKPAP_EXPERIMENTS/Statistics_comparison/R_files/DeepStat_trueX_fakeY_tables_perfx10.R")
```
...
graph_power_fdp <- ggplot(df_power_fdp, aes(x=fdp, y=power, color = KO)) + 
   ...
   scale_shape_manual(values = c(19, 0, 4, 17, 18)) +  
   geom_ribbon(aes(ymin = power - std,ymax = power + std, group = KO, fill = KO), alpha = 0.15, colour = NA) +
...
ggsave(file = "/home/julie/Documents/Paper_codes/Paper_graphs/Statistics_comparison/Figures/SM_figure_9.png", height = 7, width = 12)
...
graph_power_fdp <- ggplot(df_power_fdp, aes(x=fdp, y=power, color = KO)) + 
   ...
   geom_point(aes(shape = KO), size = 3) +
   scale_shape_manual(values = c(19, 0, 4, 17, 18)) +  # Adjust the shape values according to your preference
   geom_ribbon(aes(ymin = power - std,ymax = power + std, group = KO, fill = KO), alpha = 0.15, colour = NA) +
   geom_line(data = subset(df_power_fdp, KO == 'LASSO'), aes(group = KO))+
   #geom_line(aes(group = KO), linewidth = 1.5)+
   labs(title = "Power vs FDP, k = 10, X from X real") +
...
graph_power_fdp_fdr <-ggarrange(
  ...
  ggarrange(graph_power_fdr, graph_fdp_fdr, ncol = 2, labels = c(" ", " "), font.label = list(size = 12)), 
  nrow = 2, 
  labels = " ",
  font.label = list(size = 12)
  ) 
