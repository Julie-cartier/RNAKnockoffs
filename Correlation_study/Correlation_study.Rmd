...
graph_power_fdp <- ggplot(df_power_fdp, aes(x=fdp, y=power, color = KO)) + 
   ...
   scale_color_manual(values=c("#daa520", "#daa520", "#8fad86", "#8fad86"))  +
   scale_fill_manual(values=c("#daa520", "#daa520", "#8fad86", "#8fad86")) +
   theme(axis.text.x = element_text(size = 22, face = "bold"),
   axis.title.x = element_text(size = 24),
   axis.title.y = element_text(size = 24),
   axis.text.y = element_text(size = 22, face = "bold"),
...
ggsave(file = "/home/julie/Documents/Paper_codes/Paper_graphs/Correlation/Figures/Figure_4_new_color.png", height = 7, width = 14)  # saved for 

df_power_fdp_part <- df_power_fdp[ !grepl("BC", df_power_fdp$KO), ]

graph_power_fdp <- ggplot(df_power_fdp_part, aes(x=fdp, y=power, color = KO)) +
   ylab("Power")+
   xlab("FDP") +
   ylim(c(-0.25, 1.22)) +
   #ylim(c(-0.15, 1.20)) +
   # geom_pointrange( aes(ymin = power-std, ymax = power+std),linewidth = 1)+
   geom_point(aes(shape = KO, fill = KO, color = KO), size = 4) +
   scale_shape_manual(values = c(8,19,25, 24)) +  # Adjust the shape values according to your preference
   geom_ribbon(aes(ymin = power - std, ymax = power + std, group = KO, fill = KO), alpha = 0.15, colour = NA) +
   #geom_line(aes(group = KO), linewidth = 1.5)+
   labs(title = "Power vs FDP, k = 10, X from X real") +
   scale_color_manual(values=c("#741b47", "#741b47", "#8fad86", "#8fad86"))  +
   scale_fill_manual(values=c("#741b47", "#741b47", "#8fad86", "#8fad86")) +
   theme(axis.text.x = element_text(size = 22, face = "bold"),
   axis.title.x = element_text(size = 24),
   axis.title.y = element_text(size = 24),
   axis.text.y = element_text(size = 22, face = "bold"),
...
ggsave(file = "/home/julie/Documents/Paper_codes/Paper_graphs/Correlation/Figures/Figure_4_new_color_CRUKPAP_only.png", height = 7, width = 14)  # saved for
...
graph_power_cor <- ggplot(df_power_cor, aes(x = mean.cor, y = mean.power, color = FDP, linetype = FDP)) + 
  ...
  scale_shape_manual(values = c(22, 23, 25, 24)) + 
  scale_linetype_manual(values = c("twodash", "solid", "longdash")) +
  scale_color_manual(values = c("#0B40BF", "#BF530B", "#875580", "#c34a36")) +
  scale_fill_manual(values = c("#0B40BF", "#BF530B", "#875580", "#c34a36")) +
  theme(
    axis.text.x = element_text(size = 24, face = "bold"),
    axis.title.x = element_text(size = 28),
    axis.title.y = element_text(size = 28),
...
ggsave(file = "/home/julie/Documents/Paper_codes/Paper_graphs/Correlation/Figures/Figure_5_new_color.png", height = 7, width = 14)
